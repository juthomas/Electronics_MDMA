...
void PCD_WriteRegister(unsigned char reg, unsigned char value)
{
  SPCR = 0b1010000;
  SPSR = 0;
  ft_digital_write(RFID_SS, FT_LOW);		// Select slave
	SPDR = reg;					// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	while (!(SPSR & (1 << SPIF))) ;
  SPDR = value;
  while (!(SPSR & (1 << SPIF))) ;
	ft_digital_write(RFID_SS, FT_HIGH);		// Release slave again
}
...
unsigned char PCD_ReadRegister(unsigned char reg)
{
  ...
  ft_digital_write(RFID_SS, FT_LOW);			// Select slave
	SPDR = (0x80 | reg);
  while (!(SPSR & (1 << SPIF))); // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
                                ...
                                unsigned char rxAlign)
{
    ...
    for (i=0; i<sendLen; i++)
    {   PCD_WriteRegister(FIFODataReg, sendData[i]);    }            // FlushBuffer = 1, FIFO initialization    
    ...
    if (command == PCD_Transceive)
	  ...
	  for (i = 2000; i > 0; i--) {
		  ...
		  if (n & waitFor) {					// One of the interrupts that signal success has been set.
		  	break;
		  }
...
void PCD_init(){
  ...
  int hardReset = 0;
	...
	ft_pin_mode(RFID_SS, FT_OUTPUT);
  ft_digital_write(RFID_SS, FT_HIGH);

  ft_pin_mode(RFID_RST, FT_INPUT);

  
  if (ft_digital_read(RFID_RST) == FT_LOW) {	// The MFRC522 chip is in power down mode.
...
unsigned char PcdSelect(unsigned char *main_buf)
{
  ...
  unsigned char rxAlign = 0;
	...
	//status = rfid.PCD_CommunicateWithPICC(PCD_Transceive, &waitIrq, main_buf, bufferUsed, &buffer[6], &responseLength, &txLastBits, rxAlign);
...
unsigned char PcdAuthenticate(unsigned char blockAddr)
{
	...
	for (unsigned char i = 0; i < 6; i++) {	// 6 key unsigned chars
...
void DumpSector(unsigned char sector)
{
  ...
  while (blockIndex != 255)
  {
    ...
    for (unsigned char index = 0; index < 16; index++) {
			...
			serial_putstr(" ");
			serial_putnbr(buffer[index]); //en hexa c'est mieux
			if ((index % 4) == 3) {
				serial_putstr(" ");
			}
		}
    serial_putnbr(blockIndex);
    blockIndex--;
  }
  serial_putstr("end sector \n");
}
...
void PICC_WriteData(){
  ...
  while(i < 192)
  {
    if((block + 1) % 4)
    {
      status = MifareWrite(block, &buffer[i], 16);
      if (status != STATUS_OK)
       return;
      i += 16;
    }
